...
/**
 * ─── Xenvio Order-to-Label — Batch Flow ───────────────────────────────────────
 *
 ...
 *   XENVIO_URL        — login URL
 *   XENVIO_EMAIL      — user email
 *   XENVIO_PASSWORD   — user password
 *   APP_XENVIO        — application name to select
 *   WAREHOUSE_XENVIO  — warehouse to select
 *
 * Example usage:
 *   ORDERS_TO_CREATE=5 npx playwright test xenvio-order-to-label-batch --project=chromium
 *
 * Notes:
 ...
 *  - Individual order errors are caught and logged without aborting the batch.
 *  - Dynamic timeout: 2 minutes per order.
 */
test.describe('Xenvio Order-to-Label — Batch', () => {
    ...
    test(`TC-Xenvio-O2L-Batch: Create and label ${ordersToCreate} orders in a single session`, async ({
        ...
        await AllureHelper.applyTestMetadata({
            displayName: `Order-to-Label Batch — ${ordersToCreate} orders`,
            owner:    'QA Automation Team',
            tags:     ['xenvio', 'order-to-label', 'o2l', 'batch', 'e2e'],
            severity: 'critical',
            epic:     'Xenvio',
        ...
        console.log(`\n🚀 Starting Batch — ${ordersToCreate} order(s) to label`);
        ...
        // Track results for the final batch summary
        ...
        for (let orderIndex = 1; orderIndex <= ordersToCreate; orderIndex++) {
            // ── Session guard: recover if the popup was closed or navigated away ──
            ...
            try {
                ...
                // ── Recovery: navigate back to dashboard to clear any open panels ──
                try {
